...
#		sh -c 'cd /app && mongo --quiet $MONGO_INITDB_DATABASE /app/scripts/create_clone_indexes.js'

podman exec -e MONGO_INITDB_DATABASE="ireceptor" \
			ireceptor-database${DATABASE_CONT_NAME_SUFFIX} sh \
            -c 'cd /app && mongo --quiet $MONGO_INITDB_DATABASE /app/scripts/create_clone_indexes.js'
