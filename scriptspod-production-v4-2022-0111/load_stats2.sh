...
then
    study_id="$1" 
	metadata_file="$2"
	path_to_data_set_folder="$3"
else
    echo "usage: $0 study_id metadata_file full_path_to_data_set_folder"
    exit
fi
...
podman run --rm -it --pod=${POD_NAME_SVC} --add-host=ireceptor-database:127.0.0.1  \
	 -e DB_HOST="localhost" \
     -e DB_DATABASE="ireceptor" \
	 -e study_id="$study_id" \
	 -e metadata_file="$metadata_file" \
	 -v $path_to_data_set_folder:/study:Z \
	 -v $stats_dir:/outdir:Z \
	 -v tmp:/scratch:Z \
...
log "generated stats for $study_id"
